...
onready var VictoryScreen = preload("res://VictoryScreenEasy.tscn")
...
func _process(_delta):
	...
	#20 Level
	#273 Total mobs
	if((level == 20 && mobs_count == 252) || level==21): 
#	if(level == 2 && mobs_count == 7):
		get_tree().paused = false
		get_tree().change_scene("res://VictoryScreenEasy.tscn")
	...
	if event is InputEventMouseMotion and can_place_tower:
		...
		if not tile in invalid_tiles:
			...
			if (towertype == 1):		
				...
				tower_instance.position = tile * Vector2(65, 65)
				...
				tower_instance.position = tile * Vector2(65, 68)
				$entities.add_child(tower_instance)
				gold = gold - 150
				$user_interface/gold.text = "Gold: " + str(gold)
				$SpawnSai.play()
